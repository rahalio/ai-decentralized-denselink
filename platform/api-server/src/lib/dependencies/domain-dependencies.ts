...
const DOMAIN_PATH_PATTERNS: Array<{ pattern: RegExp; domain: string }> = [
  ...
  { pattern: /\/density\//, domain: "density" },
  { pattern: /\/grants\//, domain: "grants" },
  { pattern: /\/identity\//, domain: "identity" },
  { pattern: /\/licenses\//, domain: "licenses" },
  { pattern: /\/meshports\//, domain: "meshports" },
  { pattern: /\/reports\//, domain: "reports" },
  { pattern: /\/telemetry\//, domain: "telemetry" },
  { pattern: /\/venues\//, domain: "venues" },
];
...
export async function injectDomainDependencies(
  ...
  if (!domainDeps) {
    ...
    switch (domain) {
      case "density":
        domainDeps = buildDensityDomainModule(dynamoClient);
        break;
      case "grants":
        domainDeps = buildGrantsDomainModule(dynamoClient);
        break;
      case "identity":
        domainDeps = buildIdentityDomainModule(dynamoClient);
        break;
      case "licenses":
        domainDeps = buildLicensesDomainModule(dynamoClient);
        break;
      case "meshports":
        domainDeps = buildMeshportsDomainModule(dynamoClient);
        break;
      case "reports":
      ...
      case "telemetry":
        ...
        break;
      case "venues":
        domainDeps = buildVenuesDomainModule(dynamoClient);
        break;
      default:
        return;
    }
...
export interface DomainDependencies {
  density: DensityDomainModule;
  grants: GrantsDomainModule;
  identity: IdentityDomainModule;
  licenses: LicensesDomainModule;
  meshports: MeshportsDomainModule;
  reports: ReportsDomainModule;
  telemetry: TelemetryDomainModule;
  venues: VenuesDomainModule;
}
 ...
 */
export function getDensityDeps(
  request: FastifyRequest,
): DomainDependencies["density"] {
  return getDomainDeps(request, "density");
}
export function getGrantsDeps(
  request: FastifyRequest,
): DomainDependencies["grants"] {
  return getDomainDeps(request, "grants");
}
...
export function getLicensesDeps(
  request: FastifyRequest,
): DomainDependencies["licenses"] {
  return getDomainDeps(request, "licenses");
}
export function getMeshportsDeps(
  request: FastifyRequest,
): DomainDependencies["meshports"] {
  return getDomainDeps(request, "meshports");
}
...
export function getVenuesDeps(
  request: FastifyRequest,
): DomainDependencies["venues"] {
  return getDomainDeps(request, "venues");
}
